          ...
          <motion.div key={a.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-4 mt-2 text-xs text-henna-400">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setList(list.filter(x => x.id !== a.id))} className="px-4 py-2 bg-green-500 text-white rounded-lg text-sm font-semibold hover:bg-green-600 flex items-center gap-1"><CheckCircle size={14} /> Approve</button>
                <button onClick={() => setList(list.filter(x => x.id !== a.id))} className="px-4 py-2 bg-red-50 text-red-600 rounded-lg text-sm font-semibold hover:bg-red-100 flex items-center gap-1"><XCircle size={14} /> Reject</button>
              </div>
            </div>
          </motion.div>
        ))}
